...
class Ceremony_model extends CI_Model{
    ...
    function update($id, $data) {
        ...
        $result = $this->db->update('GRADUATE_CEREMONY_6364', $data);
        if ($result) {
            return true;
        } else {
            return false;
    ...
    function save($data) {
        $result = $this->db->insert('GRADUATE_CEREMONY_6364', $data);
    ...
    function confirm($id, $data) {
        $this->db->where('GRADUATE_CEREMONY_6364', $id);
        $result = $this->db->update('GRADUATE_CEREMONY_6364', $data);
        if ($result) {
            return true;
        } else {
    ...
    function list($params = array()){
        ...
        $query = $this->db->query("SELECT
                                ...
                                FROM GRADUATE_CEREMONY_6364 GC
                                WHERE 1=1  $auth_cond $info_cond ");
        return ($query->num_rows() > 0)?$query->result_array():FALSE;
    }

    function list6062($params = array()){
